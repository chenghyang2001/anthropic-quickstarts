...
Environment Variables:
  (none required — uses claude CLI OAuth via subscription)
  Note: If ANTHROPIC_API_KEY is set, it will be removed at startup to force OAuth.
        """,
    )
...
def main() -> None:
    ...
    # 訂閱版：清掉 ANTHROPIC_API_KEY，強制 SDK 透過 claude CLI 走 OAuth
    # （CLI 看到 env 有 API key 會優先走 API 模式，這裡明確隔離）
    if os.environ.pop("ANTHROPIC_API_KEY", None):
        print("[Subscription mode] Detected ANTHROPIC_API_KEY in env — removed for this process to force OAuth via claude CLI.")
    else:
        print("[Subscription mode] No ANTHROPIC_API_KEY — will use claude CLI OAuth (Pro/Max subscription quota).")
    print()
